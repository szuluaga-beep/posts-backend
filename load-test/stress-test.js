...
export const options = {
  stages: [
    ...
    { duration: "1m", target: 100 }, // Aumento a 1000 usuarios
    { duration: "1m", target: 100 }, // Mantener 5000 usuarios
    ...
    if (res.status === 201 || res.status === 200) {
      try {
        ...
        group("GET /posts/:id", function () {
          ...
          check(getRes, {
            "status 200": (getRes) => getRes.status === 200,
            "has data": (getRes) => JSON.parse(getRes.body).data !== undefined,
          });
        });

        sleep(0.1);

        group("PUT /posts/:id", function () {
          const updateData = {
            title: `Post estrés actualizado ${Date.now()}`,
            content: "Actualización de prueba de estrés k6",
            author: "k6-stress-test-updated",
          };
          let putRes = updatePost(postId, updateData);
          check(putRes, {
            "status 200": (putRes) => putRes.status === 200,
            "has data": (putRes) => JSON.parse(putRes.body).data !== undefined,
          });
        });
        ...
        group("DELETE /posts/:id", function () {
          ...
          check(deleteRes, {
            "status 200 or 204": (deleteRes) =>
              deleteRes.status === 200 || deleteRes.status === 204,
            "has data": (deleteRes) =>
              JSON.parse(deleteRes.body).data !== undefined,
          });
        });
      } catch (e) {
